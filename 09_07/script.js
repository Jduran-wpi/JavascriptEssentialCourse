...
const newStrapLength = (strapArray) => {
  ...
  strapArray.forEach((listElement) => {
    ...
    //change onsubmit behavior to stop refresh and change span text to the input one.
    lengthForm.addEventListener('submit', (e) =>{
      e.preventDefault();
      let input = lengthForm.querySelector('input');
      let span = listElement.querySelector('span');
      span.innerHTML = input.value? `${input.value} inches` : span.innerHTML;
      input.value = "";
    });
